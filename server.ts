...
interface Repost { id: string; postId: string; userId: string; }
interface Follow { id: string; followerId: string; followingId: string; }
interface Notification { id: string; type: NotificationType; senderId: string; receiverId: string; postId?: string; commentId?: string; seen: boolean; createdAt: string; }
...
function saveDB(db: DB) { fs.writeFileSync(DB_FILE, JSON.stringify(db, null, 2), "utf-8"); }
function makeId(prefix: string) { return `${prefix}_${crypto.randomBytes(8).toString("hex")}`; }
function normalizeEmail(email: string) { return String(email || "").trim().toLowerCase(); }
function hashValue(value: string) { return crypto.createHmac("sha256", process.env.SESSION_SECRET || "change-this-glaze-secret").update(value).digest("hex"); }
function generateOtp() { return String(crypto.randomInt(100000, 999999)); }
function usernameFromEmail(email: string, users: User[]) { const base = (email.split("@")[0].toLowerCase().replace(/[^a-z0-9_]/g, "") || "user").slice(0, 18); let name = base; let i = 1; while (users.some(u => u.username.toLowerCase() === name.toLowerCase())) name = `${base}${i++}`; return name; }
